...
class Player:
    ...
    def unowned_street(self, game, space):
        ...
        do_nothing = self.apply_action(action, game)
        if do_nothing:
    ...
    def jail_strategy(self, dice=None):
        ...
        action = self.apply_mask(action, action_mask)
        do_nothing = self.apply_action(action, game) # here if do_nothing is False it means that he neither paid nor used the card
        if do_nothing:
            if dice:
                if dice.double:
    ...
    def optional_actions(self, game):
    ...
    def spend_money(self, target, game): # target on which player will spend money
        space = self.get_space_by_action_index(target, game)
        print()
        if not space:
    ...
    def make_money(self, target, game):  # target from which player will make money
        space = self.get_space_by_action_index(target, game)
        if not space:
    ...
    def get_space_by_action_index(self, action_index, game):
    ...
    def act_on_chance(self, game, space, dice):
        pass

    def act_on_chest(self, game, space, dice):
        pass

    def act_on_tax(self, game, space, dice):
            ...
            for monopoly in self.properties:
                for space in monopoly:
                    ...
                    if space.is_mortgaged:
                        ...
                        action_mask = creditor.get_action_mask(game, mask_params)
                        state = game.get_state(creditor)
