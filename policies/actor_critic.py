...
class ActorCritic(nn.Module):
    def __init__(self, obs_shape, output_shape, hidden_size=32, nn='mlp'):
        ...
        if nn == 'resnet':
            self.model = ResNet(obs_shape, output_shape, hidden_size=hidden_size)
        elif nn == 'mlp':
            self.model = MLP(obs_shape, output_shape, hidden_size=hidden_size)
...
class ResNet(nn.Module):
    def __init__(self, obs_shape, output_size, hidden_size=64, layers=[2, 2, 2]):
